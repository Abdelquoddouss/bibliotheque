...
<section class="container px-4 mx-auto">
    ...
    @if ($users->previousPageUrl())
        <a href="{{ $users->previousPageUrl() }}" class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
    ...
    <div class="items-center hidden md:flex gap-x-3">
        @foreach (range(1, $users->lastPage()) as $page)
            <a href="{{ $users->url($page) }}" class="px-2 py-1 text-sm {{ $page == $users->currentPage() ? 'text-blue-500' : 'text-gray-500' }} rounded-md dark:bg-gray-800 {{ $page == $users->currentPage() ? 'bg-blue-100/60' : 'hover:bg-gray-100' }}">
                {{ $page }}
            </a>
        @endforeach
    </div>
    ...
    @if ($users->nextPageUrl())
        <a href="{{ $users->nextPageUrl() }}" class="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
            <span>Next</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 rtl:-scale-x-100">
